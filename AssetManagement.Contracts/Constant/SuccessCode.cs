namespace AssetManagement.Contracts.Constant
{
    public static class SuccessCode
    {
        public const string USER_REGISTER_SUCCESSFULLY = "USER_REGISTER_SUCCESSFULLY";

        public const string CREATE_USER_SUCCESSFULLY = "CREATE_USER_SUCCESSFULLY";

        public const string CREATE_ASSIGNMENT_SUCCESSFULLY = "CREATE_ASSIGNMENT_SUCCESSFULLY";

        public const string EDIT_ASSET_SUCCESSFULLY = "EDIT_ASSET_SUCCESSFULLY";

        public const string EDIT_ASSIGNMENT_SUCCESSFULLY = "EDIT_ASSIGNMENT_SUCCESSFULLY";

        public const string CHANGE_PASSWORD_SUCCESSFULLY = "CHANGE_PASSWORD_SUCCESSFULLY";
    }
}
